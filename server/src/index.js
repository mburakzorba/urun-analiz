...
const APP_VERSION = "2026-09-08-kapsamli-bilesen-kategori-kurallari";
...
app.post("/analyze", checkAppSecret, analyzeLimiter, uploadFields, async (req, res) => {
  try {
    ...
    // Kullanıcı fotoğraftan net okunamayan içerik listesini kendisi yazdıysa
    // (ScanScreen'deki opsiyonel alan), bu da SADECE fotoğraf+AI akışında
    // kullanılır ve fotoğraftan okumanın yerine geçer (daha güvenilir kabul edilir).
    ...
    // riskini yeterince azaltmıyordu. HEM en az 12 karakter HEM en az
    // 2 kelime şartı arıyorduk.
    //
    // DÜZELTME (7 Eylül 2026): 12 karakter eşiği ÇOĞU gerçek "marka + ürün
    // adı" girişini de dışarıda bırakıyordu — ör. "Axe Apollo" tam 10
    // karakter, yani önbellek hiç devreye girmiyordu. Sonuç: kullanıcı AYNI
    // ürünü iki kez taradığında, her ikisinde de AI'ye baştan soruluyordu ve
    // fotoğraftan bileşen listesini okuma (OCR benzeri bir süreç) taramadan
    // taramaya doğal olarak değişebildiği için iki taramada FARKLI bileşen
    // sayısı/riskli bileşen listesi çıkabiliyordu (kullanıcı geri bildirimi:
    // aynı ürün için bir seferinde 21, bir seferinde 15 bileşen). Eşiği artık
    // 2 kelime şartını KORUYARAK 6 karaktere indiriyoruz — "Axe Apollo" gibi
    // kısa ama yine de spesifik (marka+ürün) girişler de önbelleğe düşüyor,
    // böylece aynı ürün adıyla yapılan sonraki taramalar AI'ye YENİDEN
    // sorulmadan, İLK taramadaki (tutarlı) sonucu döner.
    const isSpecificEnoughForCache =
      !!userProvidedName && userProvidedName.length >= 6 && userProvidedName.trim().split(/\s+/).length >= 2;
